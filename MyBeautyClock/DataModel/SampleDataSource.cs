...
namespace MyBeautyClock.Data
{
    ...
    public abstract class SampleDataCommon : MyBeautyClock.Common.BindableBase
    {
    ...
    public sealed class SampleDataSource
    {
        ...
        public SampleDataSource()
        {
            ...
            var group1 = new SampleDataGroup("Group1", "時計", "時計", null, null);

            var regions1 = Regions1.Split('\n').Select((r, i) =>
                {
                    var t = r.Split(',');
                    return new { Index = i, Name = t[0], Path = t[1].TrimEnd('\r') };
                });

            foreach (var item in regions1)
            {
                group1.Items.Add(new SampleDataItem("Group1, Item" + item.Index, item.Name, "お名前", "", "", "", group1) { Path = item.Path });
            }

            this.AllGroups.Add(group1);

            var group2 = new SampleDataGroup("Group2", "スペシャル", "スペシャル", null, null);

            var regions2 = Regions2.Split('\n').Where(s => !s.Contains("男")).Select((r, i) =>
            {
                var t = r.Split(',');
                return new { Index = i, Name = t[0], Path = t[1].TrimEnd('\r') };
            });

            foreach (var item in regions2)
            {
                group2.Items.Add(new SampleDataItem("Group2, Item" + item.Index, item.Name, "お名前", "", "", "", group2) { Path = item.Path });
            }
...
千葉,chiba";
        ...
        private const string Regions2 =
@"美男,binan
美男 in 京都,binan-kyoto
カンバン男子,k-danshi
カンバン娘,k-musume
美魔女,bimajo
キッズ,kids
...
美少女,bishoujo
花嫁,hanayome
美男 in 鹿児島,binan-kagoshima
F･O･インターナショナル×キッズ,kids-fo
早稲田スタイル,wasedastyle
美男時計 in 韓流ぴあ,binan_hanryupia
グラソー　カラフル時計,glaceau_tokei
Photostudio×キッズ,kids-photostudio
Babydoll×キッズ時計,kids-babydoll";
    }
}
